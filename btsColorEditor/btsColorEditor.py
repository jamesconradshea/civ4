...
class Color:
	...
	@classmethod
	def fromFloat(cls, f):
		return min(int(f * 255 + 0.5), 255)

	@classmethod
	def fromFloats(cls, fr, fg, fb):
		return Color(Color.fromFloat(fr), Color.fromFloat(fg), Color.fromFloat(fb))
...
class ColorVals:
	...
	def setColor(self, colorName, color):
		...
		colorElem = self.getOrCreateColorElem(colorName)
		colorElem.getElementsByTagName("fRed")[0].childNodes[0].nodeValue = ("%1.17f" % fred)
		colorElem.getElementsByTagName("fGreen")[0].childNodes[0].nodeValue = ("%1.17f" % fgreen)
		colorElem.getElementsByTagName("fBlue")[0].childNodes[0].nodeValue = ("%1.17f" % fblue)
...
class App:
	...
	def __init__(self, master):
		...
		self.frame.grid()
		self.frame.winfo_toplevel().title("Civ4 BtS Color Editor 2.1: " + COLOR_EDITOR_CUSTOM_ASSETS)
		i = 0
		self.buttons = dict()
		self.selected = dict()
		for civ in self.civInfo.getCivs():
